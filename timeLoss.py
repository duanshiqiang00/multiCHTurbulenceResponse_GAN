...
#             # loss_e[i] = F.l1_loss(y_tmp ** 2, y_hat_tmp ** 2)
#             # loss_t[i] = F.l1_loss(y_tmp, y_hat_tmp)
#             loss_e[i] = MSEloss_func(y_tmp ** 2, y_hat_tmp ** 2)
#             loss_t[i] = MSEloss_func(y_tmp, y_hat_tmp)
#             if i == 0:
#                 y_phase = F.pad(y_tmp.transpose(1, 2), (1, 0), "constant", 0) - F.pad(y_tmp.transpose(1, 2), (0, 1),
#                                                                                       "constant", 0)
#                 y_hat_phase = F.pad(y_hat_tmp.transpose(1, 2), (1, 0), "constant", 0) - F.pad(y_hat_tmp.transpose(1, 2),
#                                                                                               (0, 1), "constant", 0)
#             else:
...
#             loss_p[i] = MSEloss_func(y_phase, y_hat_phase)
#
...
class TimeDomainLoss_v1(nn.Module):
    ...
    def forward(self, y, y_hat):
        ...
        MSEloss_func = nn.MSELoss()
        loss_e = MSEloss_func(y**2, y_hat**2)
        loss_t = MSEloss_func(y, y_hat)

        y_phase = F.pad(y.transpose(1, 2), (1, 0), "constant", 0) - F.pad(y.transpose(1, 2), (0, 1),
                                                                              "constant", 0)
        y_hat_phase = F.pad(y_hat.transpose(1, 2), (1, 0), "constant", 0) - F.pad(y_hat.transpose(1, 2),
                                                                                      (0, 1), "constant", 0)
        loss_p = MSEloss_func(y_phase, y_hat_phase)

        total_loss = loss_e+loss_t+loss_p
        return total_loss
